...
def calculator(request, op, a, b):
    ...
    return render(request, "calculus/calculations.html", {"result": result})


def home(request):
    # return HttpResponse(f"Dostępne operacje: {[x for x in operations]}")
    return render(
        request, "calculus/home.html", {"operations": [x for x in operations]}
    )
...
def operation(request, op):
    ...
    return render(
        ...
        {"path_example": path_example, "result": result, "op_long": op_long},
    )
